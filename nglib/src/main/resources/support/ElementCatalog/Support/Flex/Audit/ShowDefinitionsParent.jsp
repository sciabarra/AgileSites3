...
%><cs:ftcs>
<h4>Parent Template Definitions for '<ics:getvar name="assetattr"/>' FlexFamily</h4>

<ics:clearerrno/>
<ics:sql sql='<%= ics.ResolveVariables("SELECT assettype,assetattr FROM FlexGrpTmplTypes WHERE assetattr = \'Variables.assetattr\' ") %>'  table="FlexGrpTmplTypes" listname="flextmpltypes"/>
...
<ics:listloop listname="flextmpltypes">
    ...
    <tr>
        <td style="vertical-align:top;width:25%">
            <a name='<ics:resolvevariables name="templates.defname"/>'>
            <b><ics:resolvevariables name="templates.defname"/> </b></a>
            <ics:sql sql='<%= ics.ResolveVariables("SELECT assettype FROM FlexGroupTypes WHERE assetattr=\'Variables.assetattr\' AND assettemplate=\'flextmpltypes.assettype\'") %>' table="FlexGroupTypes" listname="types"/>
            <ics:listloop listname="types">
                <ics:sql sql='<%= ics.ResolveVariables("SELECT count(id) AS num FROM types.assettype WHERE flexgrouptemplateid = templates.id AND status != \'VO\'") %>' table='<%= ics.ResolveVariables("types.assettype") %>' listname="assetcount"/>
        ...
        <td style="padding:4px 0px 0px 0px">
            ...
            <% if (ics.GetErrno() == 0){ %>
                ...
                <ics:listloop listname="parentlist1">
                    <ics:clearerrno/>
                    <ics:sql sql='<%= ics.ResolveVariables("SELECT name FROM flextmpltypes.assettype WHERE id = parentlist1.productgrouptemplateid") %>' table='<%= ics.ResolveVariables("flextmpltypes.assettype") %>' listname="parentname" />
            ...
            <% } else { %>
                No attributes for this definition.
            <% } %>
