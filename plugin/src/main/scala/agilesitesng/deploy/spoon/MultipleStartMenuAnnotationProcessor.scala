...
class MultipleStartMenuAnnotationProcessor extends AbstractAnnotationProcessor[MultipleStartMenu, CtClass[_]] {
  ...
  def process(an: MultipleStartMenu, cl: CtClass[_]) {
    for (a <- an.items) {
      ...
      Spooler.insert(120, SpoonModel.StartMenu(Uid.generate(s"StartMenu.${cl.getSimpleName}.ContentForm"), name, description, "ContentForm", assetType, assetSubtype, args))
